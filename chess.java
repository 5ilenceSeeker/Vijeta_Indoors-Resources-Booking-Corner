	...
	public chess() {
		...
		backBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				setVisible(false);
				chesssearch fs= new chesssearch();
				fs.setVisible(true);
			}
		});
		...
		JLabel lblBadminton = new JLabel("Chess");
			...
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					if(validEntries()) {
						...
						String query1="select Name ,mobile ,fromm , tooo , roll , tablee from chess Where fromm= '"+ from.getText() +"' and Date = '"+ date +"' and tablee= '"+ tablee.getText() +"'";
						...
						if(count==0) {
							String query="insert into chess(Name,fromm,tooo,mobile,Date,roll,tablee) values(?,?,?,?,?,?,?)";
