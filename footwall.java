...
public class footwall extends JFrame {
	...
	public footwall() {
		...
		backBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				setVisible(false);
				footballsearch fs= new footballsearch();
				fs.setVisible(true);
			}
		});
		...
		JLabel lblBadminton = new JLabel("Football");
		...
		JLabel lblTable = new JLabel("Ground");
		lblTable.setBounds(547, 147, 130, 46);
		...
		btnSubmit.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					...
					if(validEntries()) {
						String query1="select Name ,mobile ,fromm , tooo , roll , tablee from football Where fromm= '"+ from.getText() +"' and Date = '"+ date +"'and tablee= '"+ tablee.getText() +"'";
						...
						if(count==0) {
							String query="insert into football(Name,fromm,tooo,mobile,Date,roll,tablee) values(?,?,?,?,?,?,?)";
							...
							JOptionPane.showMessageDialog(null, "Thank you for booking ground no. "+tablee.getText() + " from " + from.getText() + " to " + to.getText());
...
public boolean validEntries() {
		...
		if( (p1<9 || p2>23) ){
			JOptionPane.showMessageDialog(null, "Please Enter Valid time between 9 to 23 without timeout");
			return false;
		}
            if( p1< hour+1 || p2>23) {
